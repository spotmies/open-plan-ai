...
import type { EcoImportJobStatusDto, ImportAskUserQuestion } from '../ecoImportData';
 ...
 * issue-import/hooks/useIssueImportFlow.ts — see that file for the
 * socket/transport rationale, which applies unchanged here.
 */
export function useEcoImportFlow(projectId: string, jobId: string | null) {
  ...
  const jobQuery = useQuery<EcoImportJobStatusDto>({
    queryKey: ['eco-import-job', projectId, jobId],
    queryFn: () => ecoImportService.getStatus(projectId, jobId!),
    enabled: !!jobId,
  ...
  const conversationQuery = useQuery<ConversationDetail>({
    queryKey: ['eco-import-conversation', conversationId],
    queryFn: () => ecoImportService.getConversation(projectId, jobId!) as Promise<ConversationDetail>,
    enabled: !!conversationId,
  });

  const refetchConversation = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['eco-import-conversation', conversationId] });
  ...
  useEffect(() => {
    ...
    socket.on('ai:error', (payload: { message: string }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['eco-import-job', projectId, jobId] });
    });
  ...
  const sendMessage = useCallback(
    async (content: string): Promise<{ messageId: string } | undefined> => {
      ...
      setAssistantWorking(true);
      try {
        const result = await ecoImportService.sendMessage(projectId, jobId, content);
        refetchConversation();
        return result;
      } catch (err) {
        setAssistantWorking(false);
  ...
  const uploadAttachment = useCallback(
    async (file: File) => {
      ...
      setAssistantWorking(true);
      try {
        await ecoImportService.uploadMessageAttachment(projectId, jobId, file);
        refetchConversation();
      } catch (err) {
        setAssistantWorking(false);
  ...
  const commit = useCallback(
    async (proposalId: string) => {
      ...
      try {
        const result = await ecoImportService.commit(projectId, jobId, proposalId);
        return result;
      } finally {
        // Runs on both success AND failure — see useIssueImportFlow.ts's
        // commit() comment for why this can't be success-only.
        queryClient.invalidateQueries({ queryKey: ['eco-import-conversation', conversationId] });
        queryClient.invalidateQueries({ queryKey: queryKeys.projects.detail(projectId) });
        queryClient.invalidateQueries({ queryKey: queryKeys.ecos.all });
