...
export function ImportIssuesDialog({ open, onClose, projectId }: Props) {
  ...
  async function handleFiles(files: File[]) {
    ...
    setUploading(true);
    try {
      const job = await issueImportService.startImport(projectId, files);
      setJobId(job.jobId);
      setStage('chat');
    } catch (err) {
      setPendingFileName(null);
      ...
      <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col gap-0 p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          ...
          <DialogDescription className="text-sm">
            {stage === 'upload' && `Upload one or more files and AI will map them to issues — ${SUPPORTED_IMPORT_FILE_LABEL}.`}
            {stage === 'chat' && !isFailed && (
